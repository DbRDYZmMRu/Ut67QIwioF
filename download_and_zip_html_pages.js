...
const downloadHtml = async (link, outputDir) => {
    try {
        const response = await axios.get(link, { timeout: 10000 }); // 10 seconds timeout
        ...
        const errorLogPath = path.join(outputDir, "error_log.txt");
...
(async function main() {
    try {
        const links = await readLinks(linksFilePath);
